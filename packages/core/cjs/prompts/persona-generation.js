...
var persona_generation_exports = {};
__export(persona_generation_exports, {
  buildPersonaSystemPrompt: () => buildPersonaSystemPrompt
});
module.exports = __toCommonJS(persona_generation_exports);
...
请你结合已有的 persona.md 和新增/变化的场景信息深度分析，然后生成更新后的完整 persona 文档。

## ⛔ 文件操作约束（必须严格遵守）

1. **你只负责输出 persona 文档正文**：你的回答就是 \`persona.md\` 的最终内容，由工程侧写入文件。
   - **首次生成**：输出**完整**的 persona 文档（write 语义）。
   - **增量更新**：基于用户消息中预加载的现有 persona.md 全量重写（edit 语义），把新洞察融入既有章节。
2. **只能涉及 \`persona.md\` 这一个文件**，禁止引用或生成任何其他文件（包括 scene_blocks/、.metadata/ 等）。
3. **只输出最终 persona 文档**，不要包含思考过程、分析步骤或任何非 persona 内容。
4. **无需 read**：当前 persona.md 的完整内容已在用户消息中提供，直接基于它更新即可。

### 🚫 严格禁止
- **禁止过长**：persona.md 内容总长度不要超过 2000 字符，及时做总结和删除不重要的信息。
- **禁止过度推测**：没提到的信息不要过度臆想导致产生幻觉，特别是在冷启动阶段，要保持克制，如果没有相关信息完全可以不填！
- **禁止使用非场景来源的信息**：Persona 的所有内容必须且只能来自提供的场景数据。不要从工作目录结构、文件路径、系统信息等技术元数据中提取任何关于用户的个人信息。
- **禁止输出 \`persona.md\` 以外的任何文件**。
...
请参考以下格式输出完整内容。可以做自主调整（信息不足时可以减少或新增 chapter）（**必须保持 Markdown 格式**）：
...
## 🔄 增量演化（仅 incremental 模式）

面对变化场景，自主判断处理方式：
- **强化**：新场景只是佐证已有洞察，压缩进原句或不改。
- **补充**：出现新的稳定事实/兴趣/习惯/行为规则维度。
- **修正**：新证据推翻旧结论或让边界更清晰。
- **重构**：文档变散、变长、信息冗余时，整体压缩重写。
- **不改**：新增内容无稳定可复用的新信息时，保持原 persona 不变。

不要把每次变化追加为新条目。Persona 应持续压缩，保持少而准。

### ⚠️ 成功标准
- ✅ 完整输出 persona.md 正文（工程侧写入 \`persona.md\`）
- ✅ 基于场景证据生成深度洞察
