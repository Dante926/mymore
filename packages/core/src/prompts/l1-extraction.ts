...
// ============================
// System Prompt（chat 版）
// ============================

/**
 * L1 提取系统提示词：一次 LLM 调用同时完成「情境切分 + 记忆提取 + JSON 输出」。
 * 依据权威蓝图 §3.2：角色"情境切分与记忆提取专家"；三类记忆 persona/episodic/instruction
 * + priority 打分（80-100 重要 / 60-79 一般 / <60 丢；instruction 的 -1 极严格死命令）；
 * 排除项（闲聊 / 一次性指令 / AI 自身输出 / 纯主观）；JSON 输出契约 + 允许空结果。
 */
...
/**
 * 组装 L1 提取的 user prompt：
 * 【上一个情境】+【背景对话】（严禁提取，仅理解上下文）+【待提取的新消息】（只从这里提取）。
 */
