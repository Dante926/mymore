 ...
 */
export function buildSceneSystemPrompt(maxScenes: number = 50): string {
  const maxScenesLine =
    maxScenes > 0
      ? `**⚠️ 场景文件数量上限：${maxScenes} 个。处理完成后目录中的场景文件数量必须严格小于此上限。若已达上限，必须先 MERGE 减少文件数量（并标记 [DELETED] 删除旧文件），禁止 CREATE。**
